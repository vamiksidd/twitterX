...
const getSuggestedUser = async (req, res) => {
    try {
        ...
        // Aggregate users to exclude the current user and those already followed
        const users = await User.aggregate([
            {
                $match: {
                    _id: { $ne: userId },
                    _id: { $nin: following }  // Exclude users already followed
                }
            },
            { $sample: { size: 10 } }
        ]);
...
const followUnfollowUser = async (req, res) => {
    ...
    try {
        ...
        if (id === req.user._id.toString()) {
            return res.status(400).json({ error: "Cant follow/Unfollow yourself" })
        }
        ...
        console.log('Error cannot follow or unfollow user', err.message);
